...
function OrdersPage() {
    ...
    const handleRefund = async (orderId, maxAmount) => {
        const refundAmount = prompt(`환불 금액을 입력하세요 (최대: ${maxAmount.toLocaleString()}원)`, maxAmount);
        if (!refundAmount) return;

        const parsedAmount = parseInt(refundAmount);
        if (isNaN(parsedAmount) || parsedAmount <= 0 || parsedAmount > maxAmount) {
            alert('올바른 금액을 입력해주세요.');
            return;
        }

        const reason = prompt('환불 사유를 입력하세요', '고객 요청');
        if (!reason) return;

        if (!window.confirm(`${parsedAmount.toLocaleString()}원을 환불하시겠습니까?`)) {
            return;
        }

        try {
            await adminAPI.refundOrder(orderId, parsedAmount, reason);
            alert('환불이 완료되었습니다.');
            fetchOrders();
            setShowDetailModal(false);
        } catch (error) {
            console.error('환불 실패:', error);
            alert(error.response?.data?.message || '환불 처리에 실패했습니다.');
        }
    };
    ...
    const getStatusBadge = (status) => {
        const statusMap = {
            ...
            paid: { label: '결제완료', class: 'bg-green-100 text-green-800' },
            completed: { label: '완료', class: 'bg-blue-100 text-blue-800' },
            cancelled: { label: '취소', class: 'bg-red-100 text-red-800' },
            refunded: { label: '환불', class: 'bg-purple-100 text-purple-800' },
            partial_refunded: { label: '부분환불', class: 'bg-orange-100 text-orange-800' },
        ...
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow mb-4 sm:mb-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-2 text-gray-700">시작일</label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-2 text-gray-700">종료일</label>
                        <input
                            type="date"
                        ...
                        <div className="space-y-4 sm:space-y-6">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm sm:text-base break-all">{selectedOrder.User?.email}</div>
                                </div>
                                <div>
                                ...
                                {selectedOrder.paid_at && (
                                    <div>
                                        <div className="text-xs sm:text-sm text-gray-500">결제일시</div>
                                        <div className="text-xs sm:text-sm">{formatDate(selectedOrder.paid_at)}</div>
                                    </div>
                                )}
                            </div>
                            ...
                            {selectedOrder.refunded_amount > 0 && (
                                <div className="p-3 sm:p-4 bg-purple-50 rounded-lg border border-purple-200">
                                    ...
                                    <div className="text-sm sm:text-base">
                                        잔여금액: <span className="font-bold">
                                            {formatPrice(selectedOrder.amount - selectedOrder.refunded_amount)}
                                        </span>
                                    </div>
                                </div>
                            )}
                                    ...
                                    <>
                                        ...
                                        <button
                                            onClick={() => handleRefund(
                                                selectedOrder.id,
                                                selectedOrder.amount - (selectedOrder.refunded_amount || 0)
                                            )}
                                            className="flex-1 px-4 py-2.5 bg-orange-500 text-white rounded-lg hover:bg-orange-600 text-sm sm:text-base"
                                        >
                                            부분 환불
                                        </button>
                                    </>
                                ) : selectedOrder.status === 'partial_refunded' ? (
                                    <button
                                        onClick={() => handleRefund(
                                            selectedOrder.id,
                                            selectedOrder.amount - selectedOrder.refunded_amount
                                        )}
                                        className="flex-1 px-4 py-2.5 bg-orange-500 text-white rounded-lg hover:bg-orange-600 text-sm sm:text-base"
                                    >
                                        추가 환불
